  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      {Array.from({ length: count }).map((_, i) => (
        <span
          key={i}
          className={`h-3 w-3 rounded-full md:h-3.5 md:w-3.5 ${tone === "ink" ? "bg-ink" : "bg-[#C8C2B6]"}`}
        />
      ))}
      <span className="mx-1 text-[12px] font-bold text-muted">:</span>
      <span className="h-3 w-3 rounded-full bg-accent md:h-3.5 md:w-3.5" />
    </div>
  );
}
  ...
  return (
    <section id="elite" className="scroll-mt-20 bg-paper py-16 text-ink md:scroll-mt-24 md:py-24">
      <div className="mx-auto max-w-[1200px] px-5 md:px-6">
        ...
        <p className="mb-3 max-w-[560px] text-[16px] font-bold leading-snug tracking-tight md:text-[18px]">{elite.lead}</p>
        <p className="mb-12 max-w-[560px] text-[15px] leading-[1.75] text-paper-ink">{elite.body}</p>

        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          {elite.items.map((item) => (
            <div key={item.label} className="rounded-xl border border-line bg-white px-6 py-8">
          ...
          <div className="rounded-xl border border-dashed border-line bg-[#EFEBE3] px-6 py-8">
